      ...
      {testimonialData.map((person, index) => {
        return (
          <SwiperSlide key={index}>
            <div className='flex flex-col items-center md:flex-row gap-x-8 h-full px-4 sm:px-8 md:px-16'>
              {/* avatar, name, position */}
              <div className='w-full max-w-[300px] flex flex-col xl:justify-center items-center relative mx-auto xl:mx-0'>
                <div className='flex flex-col justify-center text-center'>
                  {/* avatar */}
                  <div className='mb-2 mx-auto relative w-[100px] h-[100px]'>
                    <Image 
                      src={person.image} 
                      alt={person.name} 
                      fill
              ...
              <div className='flex-1 flex flex-col justify-center before:w-[1px] xl:before:bg-white/20 xl:before:absolute xl:before:left-0 xl:before:h-[200px] relative xl:pl-20'>
                {/* quote icon */}
                <div className='mb-4'>
                  <FaQuoteLeft className='text-2xl sm:text-4xl xl:text-6xl text-accent/20 mx-auto md:mx-0' />
                </div>
                {/* message */}
                <div className='text-sm sm:text-base xl:text-lg text-center md:text-left italic'>
                  {person.message}
                </div>
              </div>
            </div>
